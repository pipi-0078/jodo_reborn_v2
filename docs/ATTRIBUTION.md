...
## 極楽鳥・白鵠（2026-09-18）
- ユーザー提供の鶴の参考画像をもとに内蔵画像生成でデザインを制作。参考画像自体は配布物に含めない。
- fal.ai経由 `fal-ai/hunyuan-3d/v3.1/pro/image-to-3d`。採用した再生成リクエスト `01a0b461-2bb4-71f1-a795-2f47bc73a345`。顔の輪郭と閉じたくちばしの読める斜め画像を使用。
- 画像3枚4096→2048px、JPEG quality92。元31,393,380 bytes→頭部調整後4,954,380 bytes。99,918三角形。頭・くちばしを最大15%縮小し、首へ滑らかに補間。
- 13本の骨、左右の足の接地、重心移動、休息・見回し、着水波紋をプロジェクト内で実装。翼は回転・圧縮による近似的な折り畳み。飛行未実装。
- 東側の橋近くの浅瀬に配置。水面の反射は既存の池を共有。波紋36枚のプール、専用1024px影マップ10Hz更新。
